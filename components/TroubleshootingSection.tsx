...
export default function TroubleshootingSection({
  ...
  const initialItems =
    featuredItems.length > 0 ? featuredItems : sortedItems.slice(0, 3);
  const initialItemIds = new Set(initialItems.map((item) => item.id));
  const additionalItems = sortedItems.filter(
    (item) => !initialItemIds.has(item.id),
  );
  const activeItem =
    activeItemId === null
      ? null
  ...
  const toggleExpanded = () => {
    ...
    // Keep viewport position fixed so the button naturally moves with layout.
    ...
    <div className="[overflow-anchor:none]">
      ...
      {additionalItems.length > 0 && (
        ...
        >
          {expanded
            ? "대표 항목만 보기"
            : `더보기 (${sortedItems.length - initialItems.length}개)`}
        </button>
      )}
